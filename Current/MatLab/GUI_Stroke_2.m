...
if handles.EnterData == 1
    ...
    warndlg('Please check the "Enter Data Mode" checkbox if you wish to enter data!');
end
...
elseif handles.GraphOption == 1 && handles.ArmOption == 4 && handles.InstanceState == 0
    Hand_Regular_Left_NoInstance_CSVRead()
elseif handles.GraphOption == 1 && handles.ArmOption == 5 && handles.InstanceState == 0
    Hand_Regular_Right_NoInstance_CSVRead()
...
elseif handles.GraphOption == 1 && handles.ArmOption == 4 && handles.InstanceState == 1
    Hand_Regular_Left_Instance_CSVRead()
elseif handles.GraphOption == 1 && handles.ArmOption == 5 && handles.InstanceState == 1
    Hand_Regular_Right_Instance_CSVRead()
...
elseif handles.GraphOption == 2 && handles.ArmOption == 4 && handles.InstanceState == 0
    Hand_Min_Max_Left_NoInstance_CSVRead()
elseif handles.GraphOption == 2 && handles.ArmOption == 5 && handles.InstanceState == 0
    Hand_Min_Max_Right_NoInstance_CSVRead()
...
elseif handles.GraphOption == 1 && handles.ArmOption == 4 && handles.InstanceState == 1
    Hand_Min_Max_Left_Instance_CSVRead()
elseif handles.GraphOption == 1 && handles.ArmOption == 5 && handles.InstanceState == 1
    Hand_Min_Max_Right_Instance_CSVRead()
end
guidata(hObject, handles);
...
elseif round(str2double(handles.TimeIndex)) <= 0
    ...
    try
        ...
        elseif handles.ArmOption == 4
            TimeIndent_Hand_Left_CSVRead(handles.TimeIndex);
        elseif handles.ArmOption == 5
            TimeIndent_Hand_Right_CSVRead(handles.TimeIndex);
        end
    catch
...
cnames_min_max = {'Upper Arm Phi','Upper Arm Theta','Forearm Phi', 'Forearm Theta', 'Hand Phi', 'Hand Theta'};
rnames_min_max = {'Max Angle (degrees)','Time Indent of Max','Min Angle (degrees)', 'Time Indent of Min'};
cnames = {'Time (s)', 'Upper Arm Length', 'Upper Arm Phi', 'Upper Arm Theta', 'Forearm Length', 'Forearm Phi', 'Forearm Theta', 'Hand Length', 'Hand Phi', 'Hand Theta', 'Time Indent'};

cnames_min_max_2 = {'Ring Finger', 'Index Finger', 'Thumb', 'Pinky'};
rnames_min_max_2 = {'Max Angle (degrees)','Time Indent of Max','Min Angle (degrees)', 'Time Indent of Min'};
cnames_2 = {'Time Indices', 'Ring Finger', 'Index Finger', 'Thumb', 'Pinky'};

% Create the uitable

if handles.TableOption == 1
    set(handles.CSVDisplay, 'Data', csvread('LeftArmMotionData.csv', 1, 0), 'ColumnName', cnames);
elseif handles.TableOption == 2
    set(handles.CSVDisplay, 'Data', csvread('LeftArmMotion_Min_Max_Data.csv', 1, 1), 'ColumnName', cnames_min_max, 'RowName', rnames_min_max);
elseif handles.TableOption == 3
    set(handles.CSVDisplay, 'Data', csvread('RightArmMotionData.csv', 1, 0), 'ColumnName', cnames);
elseif handles.TableOption == 4
    set(handles.CSVDisplay, 'Data', csvread('RightArmMotion_Min_Max_Data.csv', 1, 1), 'ColumnName', cnames_min_max, 'RowName', rnames_min_max);
elseif handles.TableOption == 5
    set(handles.CSVDisplay, 'Data', csvread('LeftHandMotionData.csv', 1, 0), 'ColumnName', cnames_2);
elseif handles.TableOption == 6
    set(handles.CSVDisplay, 'Data', csvread('LeftHandMotion_Min_Max_Data.csv', 1, 1), 'ColumnName', cnames_min_max_2, 'RowName', rnames_min_max_2);
elseif handles.TableOption == 7
    set(handles.CSVDisplay, 'Data', csvread('RightHandMotionData.csv', 1, 0), 'ColumnName', cnames_2);
elseif handles.TableOption == 8
    set(handles.CSVDisplay, 'Data', csvread('RightHandMotion_Min_Max_Data.csv', 1, 1), 'ColumnName', cnames_min_max_2, 'RowName', rnames_min_max_2);
end
guidata(hObject, handles);
...
elseif strcmp(handles.ArmOption, 'Superimposed Both')
    handles.ArmOption = 3;
elseif strcmp(handles.ArmOption, 'Left Hand')
    handles.ArmOption = 4;
elseif strcmp(handles.ArmOption, 'Right Hand')
    handles.ArmOption = 5;
...
elseif strcmp(handles.TableOption,'Right Arm Data')
    handles.TableOption = 3;
elseif strcmp(handles.TableOption,'Right Arm Min and Max')
    handles.TableOption = 4;
elseif strcmp(handles.TableOption,'Left Hand Data')
    handles.TableOption = 5;
elseif strcmp(handles.TableOption,'Left Hand Min and Max')
    handles.TableOption = 6;
elseif strcmp(handles.TableOption,'Right Hand Data')
    handles.TableOption = 7;
elseif strcmp(handles.TableOption,'Right Hand Min and Max')
    handles.TableOption = 8;
end
guidata(hObject, handles);
...
% --- Executes on button press in EnterDataState.
function EnterDataState_Callback(hObject, eventdata, handles)
% hObject    handle to EnterDataState (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.EnterData = get(hObject, 'Value');
guidata(hObject, handles);
% Hint: get(hObject,'Value') returns toggle state of EnterDataState
